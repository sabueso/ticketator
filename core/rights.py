...
'''
get_queues: 
Used to get all queues for a particular user, and if needed for a particular right as can_view or can_create
...
def get_queues(user, perm_level=None):
	...
	if user.username == 'admin':
		...
		if perm_level == 'can_view':
			...
			[granted_queues.append(queue.queue_dst_id) for queue in u_rights if queue.can_create== True ]
		elif perm_level == 'cv_cc':
			[granted_queues.append(queue.queue_dst_id) for queue in u_rights if queue.can_create== True or queue.can_view== True ]
	return granted_queues

'''
Designed to interact with Tickets model
Return a Q object for assigned_queue_id, used in list_tickest()
With this object we assure that only granted queus with can_view are listed as granted_queues in function
'''
